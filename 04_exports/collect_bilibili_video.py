...
if len(sys.argv) < 3:
    raise SystemExit("Usage: python collect_bilibili_video.py <BV_ID> <output_json>")
...
keywords = ["望月", "都市", "开放世界", "二游", "二次元", "米", "原神", "鸣潮", "幻塔", "绝区零", "蓝色协议", "拍档", "月灵", "探索", "跑酷", "飞行", "载具", "战斗", "实机", "玩法", "画风", "美术", "建模", "动作", "优化", "手游", "端游", "PC", "买量", "投放", "期待", "预约", "可爱", "老婆", "抄", "缝", "像", "抽卡", "氪金", "网易", "腾讯"]
...
sample = {
    "collected_at": "2026-07-28 11:10 GMT+8",
...
print("pubdate", meta.get("pubdate"))
print("comments", len(comments), "danmaku", len(barrages))
print("stat", meta.get("stat"))
print("top keywords", counter.most_common(30))
print("top comments:")
for c in sorted(comments, key=lambda x: x["like"], reverse=True)[:35]:
    print(f"[{c['source']}] like={c['like']} rc={c['reply_count']} {c['message'][:140]}")
print("top danmaku buckets", bucket.most_common(12))
